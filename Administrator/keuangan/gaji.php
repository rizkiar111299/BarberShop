...
<?php 
  $sql = mysqli_query($con,"SELECT * FROM user where level='Karyawan'");
...
echo "<table class='table table-striped table-bordered table-hover col-lg-12 col-md-6 col-sm-4 col-xs-6'>
 ...
 $tahun = $_POST['tahun'];
 $Karyawan = $_POST['kry'];
 $query = mysqli_query($con, "SELECT* from transaksi where nama_lengkap = '$Karyawan' AND month(tanggal)='$bulan' and year(tanggal) = '$tahun'");
   function rp($angka){ $angka = number_format($angka); $angka = str_replace(',', '.', $angka); $angka ="$angka"; return $angka;}
    ...
    echo "<tr><td colspan='4' style='text-align:right;'><label>Total</label></td><td style='text-align:center;'>Rp "; echo rp($hasil).''; echo "</td></tr>";
    echo "<tr><td colspan='4' style='text-align:right;'><label>Gaji Bersih</label></td><td style='text-align:center;'>Rp "; echo rp($hasil /= 2); echo "</td></tr>";
  }
?> 
</table>
                    </form>
